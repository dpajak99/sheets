...
class _SheetPinAreaLayerState extends State<SheetPinAreaLayer> {
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: <Widget>[
        ...
        Positioned(
          top: 0,
          left: rowHeadersWidth + _data.pinnedColumnsWidth - (_data.pinnedColumnCount == 0 ? pinnedBorderWidth : 0),
          width: pinnedBorderWidth,
          height: columnHeadersHeight,
          child: SheetMouseRegion(
        ...
        Positioned(
          top: columnHeadersHeight + _data.pinnedRowsHeight - (_data.pinnedRowCount == 0 ? pinnedBorderWidth : 0),
          left: 0,
          width: rowHeadersWidth,
          height: pinnedBorderWidth,
          child: SheetMouseRegion(
  ...
  void _handleColumnDragUpdate(PointerMoveEvent event) {
    ...
    setState(() {
      _cursorLeft = local.dx.clamp(rowHeadersWidth, double.infinity);
  ...
  void _handleRowDragUpdate(PointerMoveEvent event) {
    Offset local = widget.worksheet.viewport.globalOffsetToLocal(event.position);
    setState(() {
      _cursorTop = local.dy.clamp(columnHeadersHeight, double.infinity);
